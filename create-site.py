...
def create_latest_version_link(base_dir):
    ...
    all_versions = get_sorted_versions(base_dir)
...
def create_site():
    ...
    parser = argparse.ArgumentParser(description='Create a documentation site with multiple documentation versions')
    parser.add_argument('website_address', type=str, help='The address of the website')
    parser.add_argument('repo', type=str, help='The repository to download releases from')
    parser.add_argument('title', type=str, help='The title of the documentation')

    args = parser.parse_args()

    versions_cache_dir = "v"
    download_releases(args.repo, versions_cache_dir)
    ...
    with open(os.path.join(site_dir, 'index.html'), 'w') as fp:
        fp.write(load_template('index.html', website_address=args.website_address))
    ...
    canonical_header = load_template("canonical.html", website_address=args.website_address)
    ...
    for root, dirs, files in os.walk(versions_dir, topdown=True, followlinks=False):
        ...
        for file_name in files:
            ...
            elif latest_version_dir and not root.startswith(latest_version_dir):
            ...
            inject_content_into_html(file_path, VERSION_ANCHOR, get_version_bar(versions_dir, file_path, args.title))
...
def get_version_bar(versions_dir, file_name, title):

    latest_version_link_path = os.path.join(versions_dir, 'latest')

    if os.path.exists(latest_version_link_path):
        latest_version = os.path.basename(os.readlink(latest_version_link_path))
    else:
        latest_version = None
    ...
    return load_template('versions.html', versions="\n".join(versions), title=title, current_version=current_version)
